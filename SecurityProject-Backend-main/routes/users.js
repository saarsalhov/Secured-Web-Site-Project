...
router.post("/signup", async function (req, res) {
  ...
  responseExist = await con
    .promise()
    .query("call GetCountUsername(?)", [req.body.username]);
  console.log("exist: ", Object.values(responseExist[0][0])[0].cnt);
  if (Object.values(responseExist[0][0])[0].cnt !== 0) {
    res
      .status(403)
      .send(JSON.stringify({ error: "The username already exists" }));
    con.end();
    return false;
  }
  if (config.get("passwordDictonary")) {
    isPasswordInDictonaryPasswordsDb = await con
      .promise()
      .query("call CheckIfExist(?)", [req.body.confirmPassword]);
    console.log(
  ...
  var insertIntoFailedLogins = await con
    .promise()
    .query("call InsertFailLogin(?)", [userId]);
  res.send(
...
router.post("/login", async (req, res) => {
  ...
  responseExist = await con
    .promise()
    .query("call LoginCheckIfUserExist(?)", [req.body.username]);
  console.log("exist: ", Object.values(responseExist[0][0])[0].cnt);
  if (Object.values(responseExist[0][0])[0].cnt === 0) {
    res.status(400);
  ...
  console.log('***debug user id: ', userId[0][0][0].id);
  if (config.get("loginRetries") != 0) {
    ...
    console.log(
      "**** checking dateLock: ",
      Object.values(Object.values(dateLock[0][0]))
    );
    if (Object.values(Object.values(dateLock[0][0])[0])[0]) {
      console.log("there is lock need to check time");
      var isTimePassed = await con
        .promise()
        .query("call LoginRespondCountFails(?,?)", [
          config.get("lockTimeInMinutes"),
          Object.values(userId[0][0])[0].id,
        ]);
      console.log(
        "isTimePassed: ",
        Object.values(Object.values(isTimePassed[0][0])[0])[0]
      );
      if (Object.values(Object.values(isTimePassed[0][0])[0])[0] == 0) {
        ...
        console.log(
          "updading failed logins table to unlock the user! time is passed"
        );
        var updateFailedLogins = await con
          .promise()
          .query("call UpdateFailedLogin(?)", [
            Object.values(userId[0][0])[0].id,
          ]);
  ...
  await con
    .promise()
    .query("call UpdateUsersLastLogin(?)", [req.body.username]);
...
async function failedLogins(userId) {
  ...
  console.log(
    ...
    Object.values(countFailedLogins[0][0])[0].failsCount
  );
  ...
  var updatedCountFailedLogins =
    Object.values(countFailedLogins[0][0])[0].failsCount + 1;
  ...
  if (
    Object.values(countFailedLogins[0][0])[0].failsCount !=
    config.get("loginRetries")
  ) {
    console.log("updating failed logins table");
    var updateFailedLogins = await con
      .promise()
      .query("call UpdateFailsCount(?,?)", [updatedCountFailedLogins, userId]);
...
router.post("/addClient", async function (req, res, next) {
  ...
  var QueryCheckForPhoneNumber = await con
    .promise()
    .query("call GetClientWithSpecificPhoneNumber(?)", [clientPhoneNumber]);
  console.log(
    "QueryCheckForPhoneNumber",
    Object.values(QueryCheckForPhoneNumber[0][0][0])[0]
  );
  if (Object.values(QueryCheckForPhoneNumber[0][0][0])[0] === 0) {
    ...
    if (config.get("isSecured")) {
      var Query = await con
        .promise()
        .query("call GetSpecificUserFromClientsTable(?)", [clientPhoneNumber]);
      ...
      var Query = await con
        .promise()
        .query(
          "select * from clients where phoneNumber = '" +
            clientPhoneNumber +
            "';"
        );
